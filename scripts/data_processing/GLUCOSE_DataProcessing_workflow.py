...
def process_data(folder_in, folder_out):
    folder_out = os.sep.join(folder_out.split('/')[:-1])
    ...
    PrimEn.astype('float64')
    PrimEn.to_csv(my_path_csv, index=True)
    ...
    # save to csv file
    my_path_csv = Path(f'{folder_out}', 'results_processed', f'{output}'+'.csv') 
    Capacity_elc.astype('float64')
    Capacity_elc.to_csv(my_path_csv, index=True)
    ...
    for e in Emissions:
        ...
        Emissions[e].astype('float64')
        Emissions[e].to_csv(my_path_csv, index=True)
...
if __name__ == "__main__":
    ...
    # if len(args) != 2:
    #     print("Usage: python GLUCOSE_DataProcessing.py <in_folder> <out_folder> <>")
    #     exit(1)
