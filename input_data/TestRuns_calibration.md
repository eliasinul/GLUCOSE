...
## GLUCOSE_noDA2CS_14_11
Action:
- **Focus on fossil fuels electrical capacity. Adding lower constraints to fix the model historical period - so that the RDM run won't affect years 2010-2020.**
    - changing ResidualCapacity for fossil-fuel based power plants, based on existing share of techs per fuel but total capacity to match historical data from EMBER 2024 (-3%)
    - changing TotalAnnualMinCapacity for fossil-fuel based power plants, based on existing share of techs per fuel but total capacity to match historical data from EMBER 2024 (-10%)
